...
<script type="text/javascript">
	...
	var save = function(btn) {
		...
		if ($(btn).css("display") == "inline-block") {
			...
			$("#operate").attr(
					"action",
					"operateCompetition.do?method=" + method + "&idSession="
							+ idSession);
			$("#operate").submit();
		}
	}
		...
		$($(btn))
				.each(
						function() {
							...
							$("#operate").attr(
									"action",
									"operateCompetition.do?method=" + method
											+ "&id=" + id);
							$("#operate").submit();
						});
...
<body>
	<div style="text-align: center">
				...
				<form id="operate" action="" method="post">
					<c:choose>
						<c:when test="${not empty pageData.page}">
							<c:forEach var="com" items="${pageData.page}">
								<tr id="show" onmouseover="javascript:mouseOver(this);"
									...
									<td><input name="id" type="text" disabled="true"
										value="${com.id }"></td>
									<td><input name="number" type="text" disabled="true"
										value="${com.number }"></td>
									<td><input name="team_num" type="text" disabled="true"
										value="${com.team_num }"></td>
									<td><input name="name" type="text" disabled="true"
										value="${com.name }"></td>
									<td><input name="address" type="text" disabled="true"
										value="${com.address }"></td>
									<td>
		...
		<div style="margin: 0 auto">
			<form action="queryCompetition.do?count=${pageData.pageCount }"
				method="post">
