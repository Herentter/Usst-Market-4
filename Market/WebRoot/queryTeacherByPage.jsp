...
<script type="text/javascript">
	...
	var save = function(btn) {
		...
		if ($(btn).css("display") == "inline-block") {
			...
			$("#operate").attr(
					"action",
					"operateUser1.do?method=" + method + "&idSession="
							+ idSession);
			$("#operate").submit();
		}
	}
		...
		$($(btn))
				.each(
						function() {
							...
							$("#operate").attr(
									"action",
									"operateUser1.do?method=" + method + "&id="
											+ id);
							$("#operate").submit();
						});
...
<body>
	<div style="text-align: center">
				...
				<form id="operate" action="" method="post">
					<c:choose>
						<c:when test="${not empty pageData.page}">
							<c:forEach var="user" items="${pageData.page}">
								<tr id="show" onmouseover="javascript:mouseOver(this);"
									...
									<td><input name="id" type="text" disabled="true"
										value="${user.id }"></td>
									<td><input name="username" type="text" disabled="true"
										value="${user.name }"></td>
									<td><input name="password" type="text" disabled="true"
										value="${user.password }"></td>
									<td><input name="email" type="text" disabled="true"
										value="${user.email }"></td>
									<td><input name="phone" type="text" disabled="true"
										value="${user.phone }"></td>
									<td>
		...
		<div style="margin: 0 auto">
			<form
				action="queryUser.do?count=${pageData.pageCount }&select_type=teacher"
				method="post">
