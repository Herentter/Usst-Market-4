		        		...
		        		<div class="form-group">
							...
							<div class="col-md-8">
								<select class="form-control" name="type" value="${companyProduct.type}">
									...
									<option value="极致型" <c:if test="${'极致型' eq companyProduct.type}">selected</c:if>>极致型</option>
									<option value="商务型" <c:if test="${'商务型' eq companyProduct.type}">selected</c:if>>商务型</option>
									<option value="实用型" <c:if test="${'实用型' eq companyProduct.type}">selected</c:if>>实用型</option>
								</select>
							</div>
		             			...
		             			<div>
		                  			...
		                  			<c:choose>
		                  				<c:when test="${entry.key=='其他功能'}">
			                       		...
			                       		<c:otherwise>
			                       			<input id="${item.id }" type="radio" name="${item.title }"   onclick="check(this)">
			                       		</c:otherwise>
		                  			</c:choose>
			                        	${item.detail}
			                        </label>
